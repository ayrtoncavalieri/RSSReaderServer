...
#include "deleteFeed.hpp"

Poco::JSON::Object::Ptr delFeed::deleteFeed(unsigned int op, Poco::JSON::Object::Ptr req, Poco::Data::Session &session, std::string salt)
{
    Poco::JSON::Object::Ptr reqResp;
    std::string email, uuid, link;
    ...
    try{
        ...
        if(!reqResp->has("error")){
            ...
            session << "DELETE FROM `rssreader`.`links` WHERE (`email` = ?) AND (`link` = ?)", use(email), use(link), now;
        }
    }catch(...){
        throw;
    }
